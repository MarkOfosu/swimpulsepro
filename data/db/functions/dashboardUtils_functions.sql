...
BEGIN
    ...
    IF p_user_role IS NULL OR p_user_role NOT IN ('coach', 'swimmer') THEN
        RAISE EXCEPTION 'Invalid user_role. Must be either coach or swimmer';
    END IF;
        ...
        RETURN (
            SELECT json_build_object(
                'totalSwimmers', COALESCE((
                    SELECT COUNT(*)
                    FROM swimmers
                    WHERE group_id = (SELECT group_id FROM swimmers WHERE id = p_user_id)
                ), 0),
                'totalBadgesAwarded', COALESCE((
                    SELECT COUNT(*)
                    FROM swimmer_badges
                    WHERE swimmer_id = p_user_id
                ), 0),
                'attendanceRate', COALESCE((
                    SELECT ROUND(
                        (COUNT(*) FILTER (WHERE is_present = true) * 100.0 / NULLIF(COUNT(*), 0))::numeric,
...
CREATE OR REPLACE FUNCTION get_activity_feed(
    ...
    p_items_per_page INT DEFAULT 5
)
RETURNS TABLE (
    items JSON,
    total_count BIGINT
...
BEGIN
    -- Input validation
    IF p_page_number < 1 OR p_items_per_page < 1 THEN
        RAISE EXCEPTION 'Invalid pagination parameters';
    END IF;

    v_offset_val := (p_page_number - 1) * p_items_per_page;
    ...
    WITH activity_data AS (
        ...
        WHERE 
            CASE 
                WHEN p_user_role = 'coach' THEN
                    af.coach_id = p_user_id
                ELSE
                    af.group_id = (SELECT group_id FROM swimmers WHERE id = p_user_id)
            END
        ORDER BY af.created_at DESC
...
BEGIN
    ...
    RETURN COALESCE((
        ...
        WHERE CASE 
            WHEN p_user_role = 'coach' THEN
                ua.coach_id = p_user_id
            ELSE
                EXISTS (
                    SELECT 1 
                    FROM activity_groups ag
                    WHERE ag.activity_id = ua.id
                    AND ag.group_id = (SELECT group_id FROM swimmers WHERE id = p_user_id)
                )
        END
        AND ua.start_date > CURRENT_TIMESTAMP
        ORDER BY ua.start_date ASC
...
BEGIN
    ...
    VALUES (
        p_activity_data->>'title',
        p_activity_data->>'description',
        (p_activity_data->>'start_date')::TIMESTAMPTZ,
        (p_activity_data->>'end_date')::TIMESTAMPTZ,
        p_activity_data->>'location',
        p_coach_id
    )
    ...
    IF array_length(p_group_ids, 1) > 0 THEN
        INSERT INTO activity_groups (activity_id, group_id)
        SELECT v_new_activity_id, unnest(p_group_ids);
    END IF;
